...
router.get('/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const commission = await getSellerCommissionInfo();
    res.json({
...
async function finalizeCheckoutPayment({ payment_reference, buyerId }) {
  ...
  for (const item of intent.items) {
    for (const listingId of item.listing_ids) {
      ...
      ordersToCreate.push({
        listing_id: listing._id, buyer_id: buyerId, seller_id: listing.seller_id, quantity, amount,
...
router.post('/:id/confirm-delivery', authMiddleware, async (req, res) => {
  try {
    ...
    if (sellerUser) {
      ...
      const commissionInfo = await getSellerCommissionInfo();
